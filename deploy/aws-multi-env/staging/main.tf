# SchemaBot Multi-Environment — Staging Instance
#
...
variable "name_prefix" {
  ...
  type        = string
  default     = "schemabot-staging"
}


locals {
  tags = {
    Project   = "schemabot-staging"
    ManagedBy = "terraform"
  }
}
    ...
    image_repository {
      image_configuration {
        ...
        runtime_environment_variables = {
          ...
          SCHEMABOT_CONFIG_FILE = "/config/aws-multi-env-staging.yaml"
        }
      }
...
output "seed_commands" {
  description = "Commands to seed testapp staging database via bastion"
  sensitive   = true
  value       = <<-EOF
    ...
    # Terminal 2: Seed staging database
