# SchemaBot Multi-Environment — Production Instance
#
...
variable "name_prefix" {
  ...
  type        = string
  default     = "schemabot-production"
}


locals {
  tags = {
    Project   = "schemabot-production"
    ManagedBy = "terraform"
  }
}
...
resource "aws_security_group" "bastion" {
  ...
  ingress {
    ...
    to_port     = 0
    protocol    = "-1"
    cidr_blocks = ["172.31.66.0/23"] # Both private subnets
...
# Using 172.31.66.0/24 and 172.31.67.0/24 (unused in default VPC)
# -----------------------------------------------------------------------------
    ...
    image_repository {
      image_configuration {
        ...
        runtime_environment_variables = {
          ...
          SCHEMABOT_CONFIG_FILE = "/config/aws-multi-env-production.yaml"
        }
      }
...
output "seed_commands" {
  ...
  value       = <<-EOF
    ...
    # Terminal 1: Port forward to production
    ...
    # Terminal 2: Seed production database
