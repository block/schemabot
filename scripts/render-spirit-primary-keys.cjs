...
let browser;
try{
browser=await chromium.launch({headless:true,...(process.env.CHROME ? {executablePath:process.env.CHROME} : {channel:'chrome'})});
...
}finally{try{await browser?.close();}finally{fs.rmSync(framesDir,{recursive:true,force:true});}}
