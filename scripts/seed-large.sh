#!/bin/bash
# Seed testapp with data for testing schema changes.
# Connects directly to exposed MySQL ports for speed.
#
# Usage:
#   ./scripts/seed-large.sh                       # 50 MB per table, staging only
#   ./scripts/seed-large.sh 50 both               # 50 MB per table, both envs
#   ./scripts/seed-large.sh 200 production        # 200 MB per table, production only
...
STAGING_PORT=${STAGING_MYSQL_PORT:-13372}
PRODUCTION_PORT=${PRODUCTION_MYSQL_PORT:-13373}
...
# Estimate rows needed per table based on observed bytes/row (data + indexes).
# These are conservative — actual sizes will be slightly over target.
#   users:    ~163 bytes/row
