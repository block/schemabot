...
try{
 ...
 const frames=[],fps=20,duration=22;
 ...
 execFileSync('magick',['-limit','memory','512MiB','-delay','5','-loop','0',...frames,'+dither','-remap',palette,'-layers','OptimizePlus',path.join(out,name+'.gif')]);
...
(async()=>{for(const name of ['spirit-change-lifecycle','spirit-checkpoint-resume']) await render(name);})().catch(e=>{console.error(e);process.exitCode=1});
