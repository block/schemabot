...
(async()=>{
 ...
 try {
  ...
  for(let i=0;i<=204;i++){
   await page.evaluate(t=>window.renderFrame(t),sceneTime(i/12));
   if(i===0){
  ...
  execFileSync('magick',['-delay','8','-loop','0',...frames,'-layers','Optimize',path.join(root,'assets/pr-workflow-demo.gif')]);
  console.log('Rendered 205 frames: approximately 17 seconds, including 100% progress and merge');
