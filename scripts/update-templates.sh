...
#
# With --check, nothing is written: the snapshot is rendered to a temp
# file and diffed against the committed TEMPLATES.md, exiting non-zero
# on drift. CI runs the check so a hand edit or an unregenerated
# template change fails the PR instead of drifting.
...
COMMITTED="$REPO_ROOT/TEMPLATES.md"
if [ "$CHECK" = 1 ]; then
    SNAPSHOT="$(mktemp)"
...
render_markdown_section "Aggregate Check — Stopped Apply Holds the PR" "aggregate_check_stopped"
...
render_cli_section "Rate Limits (CLI + API)" "rate_limit_all"
...
if [ "$CHECK" = 1 ]; then
    if ! diff -u "$COMMITTED" "$SNAPSHOT"; then
        echo "TEMPLATES.md is stale. Run 'make templates' and commit the result." >&2
        exit 1
    fi
    echo "TEMPLATES.md is current"
fi
