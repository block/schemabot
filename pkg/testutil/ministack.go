...
const (
	...
	MiniStackImage = "ministackorg/ministack:1.5.10"

	miniStackPort = "4566"

	// miniStackStartupTimeout bounds the wait for MiniStack to register its
	// services. An emulator that has not become usable by then is a failed
	// setup rather than one worth waiting longer on, and the budget matches the
	// MySQL and PgBouncer helpers alongside this one. It is a wide margin:
	// MiniStack reports ready about a second after start, and the image is
	// pre-pulled in CI, so the pull does not come out of this budget.
	miniStackStartupTimeout = 30 * time.Second
)
