package serve

import (
	"strings"
	"testing"

	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"

	"github.com/block/schemabot/pkg/api"
)

func validLocalConfig() LocalConfig {
	return LocalConfig{
		Storage:   api.StorageConfig{DSN: "root@tcp(127.0.0.1:3306)/schemabot_state"},
		Databases: map[string]api.DatabaseConfig{"app": {Type: "mysql", Environments: map[string]api.EnvironmentConfig{"development": {DSN: "root@tcp(127.0.0.1:3306)/app"}}}},
	}
}

func TestLocalConfiguration(t *testing.T) {
	cfg, err := validLocalConfig().serverConfig()
	require.NoError(t, err)
	assert.Equal(t, "root@tcp(127.0.0.1:3306)/schemabot_state", cfg.Storage.DSN)
	assert.Equal(t, "root@tcp(127.0.0.1:3306)/app", cfg.Databases["app"].Environments["development"].DSN)
	for _, tc := range []struct {
		name    string
		mutate  func(*LocalConfig)
		message string
	}{
		{"explicit storage", func(c *LocalConfig) { c.Storage.DSN = "" }, "explicit storage"},
		{"preserve storage", func(c *LocalConfig) { c.Storage.AllowDestructiveSchemaChanges = true }, "cannot enable destructive"},
		{"separate storage", func(c *LocalConfig) { c.Storage.DSN = "other@tcp(alias:3306)/app" }, "different name"},
		{"case-insensitive separation", func(c *LocalConfig) { c.Storage.DSN = "other@tcp(alias:3306)/APP" }, "different name"},
		{"named storage", func(c *LocalConfig) { c.Storage.DSN = "root@tcp(127.0.0.1:3306)/" }, "must name a database"},
		{"MySQL storage", func(c *LocalConfig) { c.Storage.Dialect = "postgres" }, "requires MySQL storage"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			config := validLocalConfig()
			tc.mutate(&config)
			_, err := config.serverConfig()
			require.ErrorContains(t, err, tc.message)
		})
	}
}

func TestLocalListenerRequiresLoopback(t *testing.T) {
	for _, address := range []string{"0.0.0.0:8080", ":8080", "localhost:8080", "192.0.2.1:8080"} {
		err := RunLocal(t.Context(), validLocalConfig(), LocalOptions{Address: address, Token: strings.Repeat("a", 64)})
		require.ErrorContains(t, err, "numeric loopback")
	}
}
