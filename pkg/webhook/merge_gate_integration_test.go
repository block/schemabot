...
func TestE2ECheckRefreshSweepBackfillsMissedApply(t *testing.T) {
	...
	refreshReq, err := svc.Storage().CheckRefreshRequests().GetByApplyID(ctx, applyID)
	require.NoError(t, err)
	...
	h.sweepCheckRefreshRequests(ctx)
	again, err := svc.Storage().CheckRefreshRequests().GetByApplyID(ctx, applyID)
	require.NoError(t, err)
...
func TestE2ECheckRefreshReplansSiblingPRAndSkipsOriginator(t *testing.T) {
	...
	finished, err := svc.Storage().CheckRefreshRequests().GetByApplyID(t.Context(), refreshReq.ApplyID)
	require.NoError(t, err)
...
func TestE2ECheckRefreshCoalescesPendingSiblingRequests(t *testing.T) {
	...
	driven, err := svc.Storage().CheckRefreshRequests().GetByApplyID(t.Context(), first.ApplyID)
	require.NoError(t, err)
	...
	coalesced, err := svc.Storage().CheckRefreshRequests().GetByApplyID(t.Context(), second.ApplyID)
	require.NoError(t, err)
