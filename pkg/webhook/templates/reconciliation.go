...
func RenderNoManagedSchemaChangesChecksRefreshed(data NoManagedSchemaChangesChecksRefreshedData) string {
	...
	if data.GatedOnTenants {
		...
		sb.WriteString("- **An existing database with drift.** The live schema differs from the schema files for any reason, such as a change made outside SchemaBot. The files did not change, so nothing triggers a plan.\n\n")
