...
func PreviewCommentErrorGeneric() string {
	return RenderGenericError(SchemaErrorData{
		...
		Environment: "staging",
		CommandName: action.Plan,
		ErrorDetail: "Failed to prepare the schema change request. Internal SchemaBot error — retry. If it keeps failing, report error reference `" + previewErrorReference + "`.",
	})
}
...
func PreviewCommentReviewGateError() string {
	return RenderGenericError(SchemaErrorData{
		...
		ErrorDetail: "Review gate check failed. Internal SchemaBot error — retry. If it keeps failing, report error reference `" + previewErrorReference + "`. If approval is granted through a GitHub team, verify the GitHub App can read organization members and team membership.",
	})
}
