// check_refresh.go drives the check refresh guardrail around applies on a
...
const (
	...
	checkRefreshOutcomeHeld              = "held"
	checkRefreshOutcomeHoldSuperseded    = "hold_superseded"
)

// StartCheckRefreshProcessor starts the background driver that consumes
// durable check refresh requests, and registers this handler as the
// service's check refresh consumer. Registration lives here rather than at
...
// them. Idempotent; StopCheckRefreshProcessor stops the driver and waits for
...
func (h *Handler) StartCheckRefreshProcessor(ctx context.Context) {
	...
	h.service.OnCheckRefreshRecorded = h.KickCheckRefresh
...
func (h *Handler) sweepCheckRefreshRequests(ctx context.Context) {
	...
	for _, apply := range applies {
		...
		if err != nil {
			...
			// the next sweep pass.
			h.logger.Error("check refresh sweep failed to backfill a settle request for a completed apply",
		...
		if !recorded {
			...
			h.logger.Debug("check refresh sweep found the settle request already recorded",
				apply.LogAttrs()...)
			continue
		}
		h.logger.Info("check refresh sweep backfilled a settle request the drive tail did not record",
			apply.LogAttrs()...)
...
func (h *Handler) sweepPreflightedAppliesMissingSettle(ctx context.Context) {
	store := h.checkRefreshStore()
	applies, err := store.FindTerminalAppliesWithPreflightMissingSettle(ctx, h.checkRefreshSweepLookback)
	if err != nil {
		h.logger.Error("check refresh release sweep failed to find preflighted terminal applies missing a settle; held sibling PR checks stay blocked until the next pass", "error", err)
		return
	}
	for _, apply := range applies {
		recorded, err := store.Record(ctx, &storage.CheckRefreshRequest{
		...
		if err != nil {
			h.logger.Error("check refresh release sweep failed to backfill a settle request for a preflighted terminal apply; its held sibling PR checks stay blocked until the next pass",
				append(apply.LogAttrs(), "error", err)...)
			metrics.RecordCheckRefreshRecordFailure(ctx, apply.Database, apply.Environment)
			continue
		}
		if !recorded {
			h.logger.Debug("check refresh release sweep found the settle request already recorded",
				apply.LogAttrs()...)
			continue
		}
		h.logger.Info("check refresh release sweep backfilled a settle request for a preflighted terminal apply; its held sibling PR checks will be re-planned",
			apply.LogAttrs()...)
		metrics.RecordCheckRefreshRecorded(ctx, apply.Database, apply.Environment, metrics.CheckRefreshSourceReleaseSweep)
	}
}
...
// fanOutCheckRefresh dispatches a claimed request to the fan-out its kind
...
func (h *Handler) fanOutCheckRefresh(ctx context.Context, req *storage.CheckRefreshRequest) error {
	switch req.Kind {
	case storage.CheckRefreshKindPreflight:
		return h.fanOutCheckPreflight(ctx, req)
	case storage.CheckRefreshKindSettle:
	...
	default:
		...
		return fmt.Errorf("unknown check refresh request kind %q for apply %s", req.Kind, req.ApplyIdentifier)
	}
}
...
func (h *Handler) siblingChecksForTarget(ctx context.Context, req *storage.CheckRefreshRequest) ([]*storage.Check, error) {
...
// retry is safe.
func (h *Handler) fanOutCheckSettle(ctx context.Context, req *storage.CheckRefreshRequest) error {
	...
	activeHold, err := h.checkRefreshStore().HasActivePreflightedApplyOnTarget(ctx, req.Environment, req.DatabaseType, req.DatabaseName)
	if err != nil {
		return fmt.Errorf("check for active preflighted applies on target %s/%s in %s: %w",
			req.DatabaseType, req.DatabaseName, req.Environment, err)
	}
	if activeHold {
		h.logger.Info("check refresh settle deferred: a preflighted apply on the target is still active, so sibling holds stay in place until its own settle re-plans them",
			"apply_id", req.ApplyIdentifier, "environment", req.Environment,
			"database_type", req.DatabaseType, "database", req.DatabaseName)
		return nil
	}
...
func (h *Handler) fanOutCheckPreflight(ctx context.Context, req *storage.CheckRefreshRequest) error {
	targets, err := h.siblingChecksForTarget(ctx, req)
	if err != nil {
		return err
	}
	...
	h.logger.Info("check preflight holding sibling PR checks before the apply starts",
		"apply_id", req.ApplyIdentifier, "environment", req.Environment,
		"database_type", req.DatabaseType, "database", req.DatabaseName,
...
// apply finishes).
func (h *Handler) holdPRChecksForPreflight(ctx context.Context, req *storage.CheckRefreshRequest, repo string, pr int, checks []*storage.Check) error {
	...
	if prInfo.IsClosed() {
		...
		metrics.RecordCheckRefreshPROutcome(prCtx, repo, req.DatabaseName, req.Environment, checkRefreshOutcomeSkippedPRClosed)
		return nil
	}
	...
	for _, check := range actionable {
		...
		if !flipped {
			...
			metrics.RecordCheckRefreshPROutcome(prCtx, repo, req.DatabaseName, req.Environment, checkRefreshOutcomeHoldSuperseded)
			continue
		}
		held++
		metrics.RecordCheckRefreshPROutcome(prCtx, repo, req.DatabaseName, req.Environment, checkRefreshOutcomeHeld)
	}
	...
	h.logger.Info("check preflight held sibling PR checks for the target",
		"apply_id", req.ApplyIdentifier, "repo", repo, "pr", pr,
		"environment", req.Environment, "database_type", req.DatabaseType,
...
func checkHoldCommentMarker(req *storage.CheckRefreshRequest) string {
...
func (h *Handler) ensureCheckHoldComment(ctx context.Context, client *ghclient.InstallationClient, req *storage.CheckRefreshRequest, repo string, pr int) error {
