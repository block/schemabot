...
// vschemaPlans is the stored plan's per-namespace VSchema display data (see
// resolveShardedVSchemaPlans), consumed only by the sharded layout.
func formatApplyStatusComment(apply *storage.Apply, ops []*storage.ApplyOperation, released bool, tasks []*storage.Task, displayByOp map[int64]operationDisplay, shardsByTable map[string][]*storage.Task, vschemaPlans map[string]apitypes.VSchemaChange, tenant string) string {
	...
	if isShardedApply(ops) {
		return templates.RenderShardedApplyComment(buildShardedApplyData(apply, ops, released, tasks, vschemaPlans, tenant))
	}
	if len(ops) <= 1 {
...
// vschemaPlans is the stored plan's per-namespace VSchema display data (see
// resolveShardedVSchemaPlans), consumed only by the sharded layout.
func formatApplySummaryComment(apply *storage.Apply, ops []*storage.ApplyOperation, released bool, tasks []*storage.Task, displayByOp map[int64]operationDisplay, shardsByTable map[string][]*storage.Task, vschemaPlans map[string]apitypes.VSchemaChange, tenant string) string {
	...
	if isShardedApply(ops) {
		return templates.RenderShardedApplySummaryComment(buildShardedApplyData(apply, ops, released, tasks, vschemaPlans, tenant))
	}
	if len(ops) <= 1 {
