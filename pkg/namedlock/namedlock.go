...
// and PostgreSQL with pg_advisory_lock / pg_advisory_unlock; both are bound to
// a single connection's session, so every caller holds the lock on a pinned
// *sql.Conn for the lock's whole lifetime and passes that same connection to
// Release to drop the lock. A lock abandoned without Release is held until its
// session ends — and closing a pooled *sql.Conn only returns the session to
// the pool, so abandoning a lock requires discarding the connection or closing
// the owning *sql.DB.
//
...
// RELEASE_LOCK, Postgres uses pg_advisory_lock / pg_advisory_unlock.
//
...
// locks are keyed by int64, so Postgres hashes name into a key. Callers should
// treat name as an opaque identity, not a value they can read back.
type Locker interface {
	...
	// SQL failure; on error the lock is not held by conn's session. The lock
	// lives on conn's session and is dropped by Release or when the session
	// ends (closing a pooled conn returns the session to the pool — discard
	// the connection or close the owning *sql.DB to end it).
...
// Acquire and Release and keep it open for as long as the lock is held. Lock
// names are server-wide, not per-database: sessions connected to different
// databases of the same server contend on the same name. MySQL 8 rejects lock
// names longer than 64 characters; callers are responsible for keeping name
// within that bound.
type MySQL struct{}
