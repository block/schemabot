...
// relaxed by lint configuration. Callers that gate safety behavior on it must
// stay gated even for changes an operator explicitly allowed, because allowing
// an unsafe change to run is not the same as allowing it to run without its
// safety net. Where a plan is already in hand, read the unsafe verdict it
// carries instead: it is the whole linter registry's answer against the live
// schema, not this one linter's answer against the statement text.
//
