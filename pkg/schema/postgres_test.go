...
// TestPostgresTablesDeclareIdentityKey verifies every postgres storage table
// declares an identity column. The identity sequence resync refuses a target
// whose present storage tables lack one, so a table added with a serial or a
// plain integer key would turn that refusal into a false positive on every
// bootstrapped database.
func TestPostgresTablesDeclareIdentityKey(t *testing.T) {
	for name, content := range readSchemaDir(t, "postgres") {
		assert.Contains(t, content, "GENERATED BY DEFAULT AS IDENTITY",
			"%s declares no identity column", name)
	}
}
