...
// A group_finalizer dispatch applies exactly one namespace's VSchema, and the
// stored plan must carry that namespace's artifact — otherwise the operation
// would be created only to fail at drive time with nothing to apply.
func TestFinalizerDispatchNamespace(t *testing.T) {
	plan := &storage.Plan{
		ID: 7,
	...
	t.Run("valid namespace", func(t *testing.T) {
		ns, err := finalizerDispatchNamespace(plan, []string{"ks_a"})
		require.NoError(t, err)
		assert.Equal(t, "ks_a", ns)
	})
	t.Run("multiple namespaces fail closed", func(t *testing.T) {
		_, err := finalizerDispatchNamespace(plan, []string{"ks_a", "ks_b"})
		require.Error(t, err)
		assert.Contains(t, err.Error(), "expected exactly 1")
	})
	t.Run("namespace without a VSchema artifact fails closed", func(t *testing.T) {
		_, err := finalizerDispatchNamespace(plan, []string{"ks_b"})
		require.Error(t, err)
