...
| Resuming | `resuming` | A stopped apply has been claimed for a start and the data plane has accepted the start, but has not yet left the stopped state. Transient: advances to `running` once the data plane resumes, or back to `stopped` if the resume does not take within the grace window (remote-engine resumes only) |
...
stateDiagram-v2
    ...
    stopped --> resuming : start accepted (remote engine)
    resuming --> running : data plane left stopped
    resuming --> stopped : resume grace window expired
...
- `resuming`: Remote-engine (gRPC) only. After a stopped apply is claimed for a start and the data plane accepts the start, the apply is held in `resuming` until the data plane actually reports leaving the stopped state. This keeps `/api/status` and `/api/progress/apply/{id}` consistent: both serve `resuming` from storage instead of one showing `running` while live remote progress still reports `stopped`. It is non-terminal and active, so the one-active-apply overlap invariant keeps the target reserved and recovery can reclaim a stale resuming apply. It advances to `running` once the data plane leaves stopped, or back to `stopped` if the data plane stays stopped past the start grace window.
