...
- `waiting_for_cutover`/`cutting_over`: With `--defer-cutover` or atomic mode (Spirit). PlanetScale applies pass through both even without `--defer-cutover`: the deploy request is created with the backend's auto-cutover disabled, so it parks at pending cutover and the drive — the sole cutover actor — triggers cutover automatically. Note: `--defer-cutover` is a no-op for instant DDL (no cutover exists).
