...
func (m *initWizard) discoverNamespaces() tea.Cmd {
	...
	return func() tea.Msg {
		...
		if engine != "vitess" {
			...
			var err error
			target.Token, err = m.resolveConnection(tokenRef)
			if err != nil {
				return initNamespacesMsg{generation: generation, err: fmt.Errorf("choose a PlanetScale token reference before discovering keyspaces")}
			}
		}
		target.DSN = dsn
