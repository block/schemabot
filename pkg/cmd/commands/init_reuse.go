...
func stageExistingInitSchema(root, stage, database, engine, environment string, namespaces []string) (client.PlanExclusions, error) {
	snapshot, err := initSchemaSnapshot(root)
	if err != nil {
		return client.PlanExclusions{}, err
	}
	if _, exists := snapshot["schemabot.yaml"]; !exists {
		return client.PlanExclusions{}, fmt.Errorf("schemabot.yaml not found in %q; choose a schema directory with a valid configuration", root)
	}
	...
	if !slices.Equal(actual, selected) {
		return client.PlanExclusions{}, fmt.Errorf("existing schema namespaces %v do not match selected scope %v", actual, selected)
	}
