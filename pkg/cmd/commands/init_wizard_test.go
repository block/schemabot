...
func TestInitWizardNavigationAndValidation(t *testing.T) {
	...
	require.Equal(t, "postgres", m.fields[stepEngine].value)
...
func TestInitWizardDetectsExistingProjectBeforeReview(t *testing.T) {
	...
	m := newInitWizard(&InitCmd{SchemaDir: root}, "default", io.Discard)
	require.Equal(t, "shop", m.fields[stepName].value)
	require.Equal(t, "mysql", m.fields[stepEngine].value)
	...
	require.Equal(t, "postgres", explicit.fields[stepEngine].value)
	require.Equal(t, "other", explicit.fields[stepName].value)
}
...
func TestInitWizardVitessStepsAndReview(t *testing.T) {
	...
	require.Equal(t, len(m.fields), m.step)
	view := m.View()
	require.Contains(t, view, "Organization")
	require.Contains(t, view, "acme")
	require.NotContains(t, view, "tok-secret")
	wizardKey(m, tea.KeyEnter)
	require.True(t, m.confirmed)
