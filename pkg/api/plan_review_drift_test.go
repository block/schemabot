...
func TestRollupReviewTimeDrift_DivergingDeploymentBlocks(t *testing.T) {
	...
	rollup, err := svc.RollupReviewTimeDrift(t.Context(), planDiffReq(t), reviewed, productionMember("eu"))
	require.NoError(t, err)
...
func TestRollupReviewTimeDrift_UnresolvedTargetsError(t *testing.T) {
	...
	_, err := svc.RollupReviewTimeDrift(t.Context(), req, reviewedUsersPlan("ALTER TABLE `users` ADD COLUMN `email` varchar(255)"), productionMember("eu"))
	require.Error(t, err)
}
