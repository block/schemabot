...
//
// It answers from the live database and from the embedded schema files of the
// binary that serves the request — never from a version pin. A consumer's
// go.mod pin says which release a host binary *was built against*; it says
// nothing about what the storage it talks to has actually converged to, and the
// two diverge exactly when a deploy has failed to converge. Since that is
// precisely when someone computes this diff, the pin is the one input that
// cannot be trusted, so no input to this package is a version.
//
// The diff is the bootstrap's own diff, not a second implementation of it. On
// MySQL that is Spirit's differ over readEmbeddedSchemaFiles; on PostgreSQL it
// is postgresSchemaDriftFor over the embedded PostgreSQL files. A statement
// this reports is a statement a boot of this binary would plan, because it came
// from the same call.
...
// to match the embedded schema of the binary that produced the report.
//
...
type StorageSchemaReport struct {
	...
	// files produced the diff. It is reported, never consumed: the diff is
	// computed from the files themselves, and this only says whose files they
	// were.
	Version string
...
// DiffStorageSchema reports the storage DDL outstanding between the embedded
// schema files of this binary and the live storage database at dsn. It is
// strictly read-only: it opens connections, reads the catalog, and computes a
// diff. It executes no DDL, takes no advisory lock, and writes nothing, so it
// is safe to run at any time, including against a database an apply is
...
func DiffStorageSchema(ctx context.Context, dsn string, logger *slog.Logger, opts ...EnsureSchemaOption) (*StorageSchemaReport, error) {
	o := newEnsureSchemaOptions(opts...)
	switch o.dialect {
	case schema.DialectMySQL:
		return diffMySQLStorageSchema(ctx, dsn, o)
	case schema.DialectPostgres:
		return diffPostgresStorageSchema(ctx, dsn, o)
	default:
...
func ApplyStorageSchema(ctx context.Context, dsn string, logger *slog.Logger, opts ...EnsureSchemaOption) (planned, remaining *StorageSchemaReport, err error) {
	planned, err = DiffStorageSchema(ctx, dsn, logger, opts...)
	if err != nil {
	...
	remaining, err = DiffStorageSchema(ctx, dsn, logger, opts...)
	if err != nil {
...
// diffMySQLStorageSchema diffs the embedded MySQL schema files against the live
...
func diffMySQLStorageSchema(ctx context.Context, dsn string, o ensureSchemaOptions) (*StorageSchemaReport, error) {
	report := &StorageSchemaReport{Dialect: schema.DialectMySQL, DestructiveAllowed: o.allowDestructive}

	// The database name is what makes the report readable as being about one
	// database. Unlike the bootstrap's preamble, a failure here is fatal: the
	// bootstrap can converge without knowing the name, but a report that cannot
	// say which database it read is one an operator cannot act on.
...
// diffPostgresStorageSchema diffs the embedded PostgreSQL schema files against
...
func diffPostgresStorageSchema(ctx context.Context, dsn string, o ensureSchemaOptions) (*StorageSchemaReport, error) {
	report := &StorageSchemaReport{Dialect: schema.DialectPostgres}
	...
	if err := db.QueryRowContext(ctx, "SELECT current_database()").Scan(&report.Database); err != nil {
		return nil, fmt.Errorf("read storage target identity: %w", err)
	}
