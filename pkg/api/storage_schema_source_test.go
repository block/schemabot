...
func TestStorageSchemaFromFiles_Validation(t *testing.T) {
	tests := []struct {
		...
		{
			// The phrase is how a report says the files are the answering
			// binary's own, and AttributeTo rewrites it to that binary's
			// version. A supplied set wearing it would come back labelled with
			// a release that did not produce it.
			name:        "wearing the embedded schema's description",
			description: embeddedStorageSchemaDescription,
			files:       validStorageSchemaFiles(),
			wantErr:     "is reserved for a binary's own embedded schema",
		},
	}
