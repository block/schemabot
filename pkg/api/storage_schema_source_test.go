...
func TestValidateStorageSchemaDiffRequest(t *testing.T) {
	...
	err := validateStorageSchemaDiffRequest(apitypes.StorageSchemaDiffRequest{SchemaFiles: validStorageSchemaFiles()})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "without schema_source")

	err = validateStorageSchemaDiffRequest(apitypes.StorageSchemaDiffRequest{SchemaSource: "the schema files of release v1.4.0"})
	require.Error(t, err)
