...
type StorageSchemaSource struct {
	...
	Description string
	// Files is the schema, as file name → file contents.
	Files map[string]string
	// embedded marks the one source whose files are not carried here because
	// they are compiled in: the running binary's own. Everything else has to
	// bring its files, so a source that names a release and carries nothing is
	// an error rather than this binary's schema under that release's name.
	embedded bool
}
...
func EmbeddedStorageSchema(version string) *StorageSchemaSource {
	...
	return &StorageSchemaSource{Description: description, embedded: true}
}
...
func StorageSchemaFromFiles(description string, files map[string]string) (*StorageSchemaSource, error) {
	...
	if description == embeddedStorageSchemaDescription {
		// The embedded description is how a report says "these are the answering
		// binary's own files", and AttributeTo rewrites it to that binary's
		// version. A supplied set wearing it would be relabelled with a version
		// that did not produce it, which is the misattribution this whole
		// surface exists to prevent — so the phrase is reserved.
...
// isEmbedded reports whether the diff should read this binary's compiled-in
// schema files. No source at all is the embedded schema — that is how a boot
// asks — and so is one EmbeddedStorageSchema built.
func (s *StorageSchemaSource) isEmbedded() bool {
	return s == nil || s.embedded
}

// checkSupplied refuses a source that carries no files to diff against. The
// alternative — reading the running binary's own files instead — produces a
// report computed from one release and attributed to another, which is the
// misattribution this whole surface exists to prevent, and it is reached by an
// empty map and by a nil one alike. StorageSchemaFromFiles already refuses this
// at the door; this holds the same line for a source built as a struct literal.
func (s *StorageSchemaSource) checkSupplied() error {
	if len(s.Files) > 0 {
		return nil
	}
...
func (s *StorageSchemaSource) mysqlSchemaFiles() (schema.SchemaFiles, error) {
	...
	if err := s.checkSupplied(); err != nil {
		return nil, err
	}
