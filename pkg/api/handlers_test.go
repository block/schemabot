...
func TestExecuteApplyMarksCreatedTasksFailedWhenQueueingFails(t *testing.T) {
	...
	assert.Contains(t, err.Error(), "store task for table orders")
	require.NotNil(t, applies.apply)
	assert.Equal(t, state.Apply.Failed, applies.apply.State)
	assert.Contains(t, applies.apply.ErrorMessage, "task insert failed")
	require.Len(t, tasks.tasks, 1)
	assert.Equal(t, state.Task.Failed, tasks.tasks[0].State)
	assert.Contains(t, tasks.tasks[0].ErrorMessage, "task insert failed")
	assert.NotNil(t, tasks.tasks[0].CompletedAt)
}
