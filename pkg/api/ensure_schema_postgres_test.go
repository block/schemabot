...
func TestPostgresManualRemediation(t *testing.T) {
	...
	mixed := postgresSchemaDrift{
		...
		"settings": {
			...
			{operation: "create_index", object: "idx_settings_setting_key", manualReason: "live index is non-unique where the embedded schema requires a unique index; replace it manually"},
		},
	}
	...
	assert.Contains(t, err.Error(), `storage table "settings" has index "idx_settings_setting_key" whose live index is non-unique`)
}
...
func TestPostgresLiveIndexManualReason(t *testing.T) {
	...
	assert.Contains(t, postgresLiveIndexManualReason(unique, postgresLiveIndex{unique: false, valid: true}), "live index is non-unique")
	assert.Contains(t, postgresLiveIndexManualReason(unique, postgresLiveIndex{unique: true, valid: false}), "live index is invalid")
	assert.Contains(t, postgresLiveIndexManualReason(nonUnique, postgresLiveIndex{unique: false, valid: false}), "live index is invalid")
}
