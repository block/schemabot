...
func (s *vitessApplyDataStore) Save(ctx context.Context, data *storage.VitessApplyData) error {
	_, err := s.db.ExecContext(ctx, `
		INSERT INTO vitess_apply_data (apply_id, branch_name, deploy_request_id, migration_context, deploy_request_url, revert_skipped_at)
		VALUES (?, ?, ?, ?, ?, ?)
		...
		data.ApplyID, data.BranchName, data.DeployRequestID, data.MigrationContext, data.DeployRequestURL, data.RevertSkippedAt,
	)
	if err != nil {
...
func (s *vitessApplyDataStore) GetByApplyID(ctx context.Context, applyID int64) (*storage.VitessApplyData, error) {
	...
	err := s.db.QueryRowContext(ctx, `
		SELECT apply_id, branch_name, deploy_request_id, migration_context, deploy_request_url, revert_skipped_at
		FROM vitess_apply_data WHERE apply_id = ?`, applyID,
	).Scan(&data.ApplyID, &data.BranchName, &data.DeployRequestID, &data.MigrationContext, &data.DeployRequestURL, &data.RevertSkippedAt)
	if err != nil {
