...
const checkRefreshColumns = `id, apply_id, kind, apply_identifier, environment, database_type, database_name,
...
func (s *checkRefreshRequestStore) Record(ctx context.Context, req *storage.CheckRefreshRequest) (bool, error) {
	...
	if req.Kind != storage.CheckRefreshKindPreflight && req.Kind != storage.CheckRefreshKindSettle {
		return false, fmt.Errorf("check refresh request for apply %s has unknown kind %q", req.ApplyIdentifier, req.Kind)
	}
	...
	if err != nil {
		...
		return false, fmt.Errorf("record %s check refresh request for apply %s (%s/%s in %s): %w",
...
func (s *checkRefreshRequestStore) GetByApplyAndKind(ctx context.Context, applyID int64, kind string) (*storage.CheckRefreshRequest, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("get %s check refresh request for apply row %d: %w", kind, applyID, err)
	}
	return req, nil
}
...
func (s *checkRefreshRequestStore) ReopenForRetry(ctx context.Context, id int64) (bool, error) {
	result, err := s.db.ExecContext(ctx, `
		...
		SET state = ?, attempts = 0, lease_owner = NULL, lease_token = NULL,
			...
			updated_at = NOW()
		WHERE id = ? AND state = ?
	`, storage.CheckRefreshPending, id, storage.CheckRefreshFailed)
	if err != nil {
		return false, fmt.Errorf("reopen check refresh request %d for retry: %w", id, err)
	}
	rows, err := result.RowsAffected()
	if err != nil {
		return false, fmt.Errorf("read reopened check refresh request %d rows affected: %w", id, err)
	}
	return rows > 0, nil
}
...
func (s *checkRefreshRequestStore) PendingForTarget(ctx context.Context, environment, databaseType, databaseName, kind string, excludeID int64) ([]*storage.CheckRefreshRequest, error) {
	rows, err := s.db.QueryContext(ctx, `
		...
		ORDER BY created_at, id
	`, environment, databaseType, databaseName, kind, storage.CheckRefreshPending, excludeID)
	if err != nil {
		return nil, fmt.Errorf("query pending %s check refresh requests for %s/%s in %s: %w",
...
func (s *checkRefreshRequestStore) FindCompletedAppliesMissingRequest(ctx context.Context, lookback time.Duration) ([]*storage.Apply, error) {
	...
	rows, err := s.db.QueryContext(ctx, `
		...
		FROM applies a
		LEFT JOIN check_refresh_requests r ON r.apply_id = a.id AND r.kind = ?
		WHERE a.state = ?
	...
	`, storage.CheckRefreshKindSettle, state.Apply.Completed, int64(lookback.Seconds()))
	if err != nil {
		return nil, fmt.Errorf("query completed applies missing settle check refresh requests: %w", err)
	}
	defer utils.CloseAndLog(rows)
	...
	for rows.Next() {
		...
		if err != nil {
			return nil, fmt.Errorf("scan completed apply missing settle check refresh request: %w", err)
		}
...
// settle. Their preflight fan-out held sibling PR checks action-required, and
...
func (s *checkRefreshRequestStore) FindTerminalAppliesWithPreflightMissingSettle(ctx context.Context, lookback time.Duration) ([]*storage.Apply, error) {
	if lookback <= 0 {
		return nil, fmt.Errorf("check refresh sweep lookback must be positive")
	}
	terminalStates := terminalApplyStates()
	args := []any{storage.CheckRefreshKindPreflight, storage.CheckRefreshKindSettle}
	args = append(args, stringArgs(terminalStates)...)
	args = append(args, int64(lookback.Seconds()))
	rows, err := s.db.QueryContext(ctx, `
		SELECT `+applyColumnsForApplyAlias+`
		FROM applies a
		JOIN check_refresh_requests pre ON pre.apply_id = a.id AND pre.kind = ?
		LEFT JOIN check_refresh_requests settle ON settle.apply_id = a.id AND settle.kind = ?
	...
	`, args...)
	if err != nil {
		return nil, fmt.Errorf("query terminal applies with preflight missing settle check refresh requests: %w", err)
	}
	defer utils.CloseAndLog(rows)
	...
	for rows.Next() {
		apply, err := scanApplyInto(rows)
		if err != nil {
			return nil, fmt.Errorf("scan terminal apply with preflight missing settle check refresh request: %w", err)
		}
		applies = append(applies, apply)
	}
	return applies, rows.Err()
...
// holding) sibling PR checks and is guaranteed a settle of its own once it
...
func (s *checkRefreshRequestStore) HasActivePreflightedApplyOnTarget(ctx context.Context, environment, databaseType, databaseName string) (bool, error) {
	nonTerminal, nonTerminalArgs := nonTerminalApplyStatePredicate("a.state")
	args := []any{storage.CheckRefreshKindPreflight, environment, databaseType, databaseName}
	args = append(args, nonTerminalArgs...)
	var exists bool
	err := s.db.QueryRowContext(ctx, `
		SELECT EXISTS (
			SELECT 1
			FROM check_refresh_requests pre
