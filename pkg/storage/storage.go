...
type ApplyStore interface {
	...
	// drive only the remaining work. The caller passes the logical lock row it
	// acquired for the target; the transition records that lock_id for association
	// and re-checks active-apply exclusivity under the apply-target advisory lock —
	// that exclusivity re-check, not lock-row validation, is the concurrency guard
	// that keeps making a terminal apply active again safe.
	ReapplyFailed(ctx context.Context, applyID int64, lockID int64) (*Apply, error)
