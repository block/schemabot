...
// TestPlanCommentStore_MarkDeletedPreservesStamp reads the raw deleted_at
// column to prove a repeat mark does not move the original stamp. The row is
// backdated between marks because the column's whole-second precision would
// otherwise let a re-stamp within the same second pass unnoticed.
func TestPlanCommentStore_MarkDeletedPreservesStamp(t *testing.T) {
	clearTables(t)
	ctx := t.Context()
	store := NewMySQL(testDB)
	...
	require.NoError(t, testDB.QueryRowContext(ctx,
		"SELECT deleted_at FROM plan_comments WHERE id = ?", comment.ID).Scan(&deletedAt))
	require.NotNil(t, deletedAt, "the row is stamped, not removed")

	_, err := testDB.ExecContext(ctx,
		"UPDATE plan_comments SET deleted_at = DATE_SUB(NOW(), INTERVAL 1 HOUR) WHERE id = ?", comment.ID)
	require.NoError(t, err)
	var backdated *time.Time
	require.NoError(t, testDB.QueryRowContext(ctx,
		"SELECT deleted_at FROM plan_comments WHERE id = ?", comment.ID).Scan(&backdated))
	require.NotNil(t, backdated)
	...
	var afterRepeat *time.Time
	require.NoError(t, testDB.QueryRowContext(ctx,
		"SELECT deleted_at FROM plan_comments WHERE id = ?", comment.ID).Scan(&afterRepeat))
	require.NotNil(t, afterRepeat)
	assert.Equal(t, *backdated, *afterRepeat, "a repeat mark must not move the stamp")
}
