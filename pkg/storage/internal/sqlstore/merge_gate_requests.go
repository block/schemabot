...
// canonicalizeMergeGateRequest folds the request's identity keys in place.
// The target triple is what the fan-out and the sibling coalescing look rows
// up by, so an unfolded spelling on a byte-comparing dialect would hide a
// recorded request from every one of those lookups.
func canonicalizeMergeGateRequest(req *storage.MergeGateRequest) {
	if req == nil {
		return
	}
	req.Environment = storage.CanonicalKey(req.Environment)
	req.DatabaseType = storage.CanonicalKey(req.DatabaseType)
	req.DatabaseName = storage.CanonicalKey(req.DatabaseName)
...
func (s *mergeGateRequestStore) Record(ctx context.Context, req *storage.MergeGateRequest) (bool, error) {
	canonicalizeMergeGateRequest(req)
	if req.ApplyID == 0 {
