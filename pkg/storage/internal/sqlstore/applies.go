...
func (s *applyStore) ExpireRetryable(ctx context.Context) ([]*storage.RetryableApplyExpiration, error) {
	...
	// A fresh operation lease can belong to a live redispatch even when its
	// state is failed_retryable. Protect its tasks regardless of operation state.
	// Once its lease is stale, the task reapers can settle any skipped rows
	// against the parent verdict written below.
	unleasedOperation := unleasedOperationGate(s.dialect)
	...
	_, err = tx.ExecContext(ctx, fmt.Sprintf(`
		...
		WHERE state = ? AND apply_id IN (%s)
			AND %s
	`, placeholders(len(applyIDs)), unleasedOperation), cancelArgs...)
	if err != nil {
	...
	_, err = tx.ExecContext(ctx, fmt.Sprintf(`
		...
		WHERE state NOT IN (%s) AND apply_id IN (%s)
			AND %s
	`, placeholders(len(state.TerminalTaskStates)), placeholders(len(applyIDs)), unleasedOperation), taskArgs...)
	if err != nil {
