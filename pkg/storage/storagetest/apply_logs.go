...
func TestApplyLogs(t *testing.T, h Harness) {
	...
	// chronological order, even when entries share a created_at second (id
	// breaks the tie so insertion order is preserved).
	...
	// busy apply sees the latest activity, not a stale head. Ties on
