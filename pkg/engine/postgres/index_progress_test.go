...
func TestConcurrentIndexPercent(t *testing.T) {
	cases := []struct {
		...
		{name: "btree initializing sits at the start", phase: "building index: initializing", want: 0, derived: true},
		{name: "sorting live tuples sits at 40", phase: "building index: sorting live tuples", want: 40, derived: true},
		{name: "sorting dead tuples sits at 40", phase: "building index: sorting dead tuples",
			work: progress.Work{BlocksDone: 40, BlocksTotal: 40}, want: 40, derived: true},
		...
		{name: "loading tuples ignores the carried heap scan blocks", phase: "building index: loading tuples in tree",
			work: progress.Work{BlocksDone: 40, BlocksTotal: 40, TuplesDone: 0, TuplesTotal: 20}, want: 40, derived: true},
		...
		{name: "index scan ignores the carried tuple counters", phase: "index validation: scanning index",
			work: progress.Work{TuplesDone: 20, TuplesTotal: 20}, want: 60, derived: true},
		...
		{name: "a phase only a reindex reports sits at the end of the scale", phase: "waiting for readers before dropping",
			work: progress.Work{BlocksDone: 40, BlocksTotal: 40}, want: 95, derived: true},
