...
func TestEngine_Progress_FailingApplyNeverReportsCompleted(t *testing.T) {
	...
	go func() {
		defer close(done)
		eng.executeSchemaChange(ctx, dsn, host, username, password, database, ddlStatements, false, direct.Policy{})
	}()
...
func TestEngine_ExecuteMigration_SingleStatementReleasesConnections(t *testing.T) {
	...
	for i := range iterations {
		...
		eng.executeSchemaChange(t.Context(), dsn, host, username, password, database, []string{createDDL}, false, direct.Policy{})
		...
		eng.executeSchemaChange(t.Context(), dsn, host, username, password, database, []string{dropDDL}, false, direct.Policy{})
