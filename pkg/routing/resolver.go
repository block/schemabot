...
//
// It asks MultiTargetDeployments which deployments those are rather than
// repeating the test, so the name an operator reads and the key an operation is
// stored under cannot disagree about when a target is worth naming.
