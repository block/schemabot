...
| `schemabot.target.probe.total` | Counter | database_type, environment, outcome | Startup connectivity probes of enumerated targets, by outcome. This counter sits below the volume bar in [Does it belong in a metric?](#does-it-belong-in-a-metric) — one observation per target per process start — and earns its place as a deploy-time alert instead: a fleet-wide rollout produces a burst of observations at once, and any non-`success` outcome in that burst is a rotated credential, dropped grant, or unreachable target found before the first schema change reaches it. Alert on `outcome != success` per environment after each deploy. Target is omitted to bound cardinality; the paired warn log names it |
...
| `schemabot.target.client_evictions.total` | Counter | database_type, environment, reason | Data-plane client generations the target router replaced. `dsn_changed` is a resolver returning a new connection identity for a route (a credential rotation under `dsn_from`); a rate well above the rotation cadence means the resolver's DSN is unstable and clients are being rebuilt per request. `auth_invalid_credentials`, `auth_no_access`, and `auth_no_database` are a routed read failing target authentication on the generation, which the router evicts before its single retry; alert on these as well as `dsn_changed` to see credential failures. Target is omitted to bound cardinality; the paired info log names it |
| `schemabot.target.auth_retries.total` | Counter | operation, database_type, environment, classification, outcome | Single automatic retries of routed reads after an authentication-classified target failure, by outcome: `success`, `failed`, `resolve_error` (the target could not be re-resolved or its client rebuilt), or `suppressed` (a failed retry armed the route's cooldown and this read returned the failure without retrying). A sustained `failed` or `suppressed` rate means target credentials or grants are unstable; target is omitted to bound cardinality and the paired log names it |
