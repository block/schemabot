...
| `schemabot.target.client_evictions.total` | Counter | database_type, environment, reason | Data-plane client generations the target router replaced. `dsn_changed` is a resolver returning a new connection identity for a route (a credential rotation under `dsn_from`); a rate well above the rotation cadence means the resolver's DSN is unstable and clients are being rebuilt per request. Target is omitted to bound cardinality; the paired info log names it |
| `schemabot.target.auth_retries.total` | Counter | operation, database_type, environment, classification, outcome | Single automatic retries of routed reads after an authentication-classified target failure, by outcome. A sustained rate means target credentials or grants are unstable; target is omitted to bound cardinality and the paired log names it |
