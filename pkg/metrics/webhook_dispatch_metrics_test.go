...
// newWebhookMetricsReader installs a manual OTel reader for the test's
// lifetime so the recorded dispatch metrics can be collected and asserted.
func newWebhookMetricsReader(t *testing.T) *metric.ManualReader {
	t.Helper()
...
func TestRecordWebhookInboxDispatchLagClampsAndFolds(t *testing.T) {
	...
	assert.Equal(t, "unknown", metricAttributeString(t, points[0].Attributes, "event_type"))
}
...
func TestRecordWebhookDispatchDurationFoldsUnknownOutcome(t *testing.T) {
	...
	points := collectHistogramPoints(t, reader, "schemabot.webhook.dispatch_duration_seconds")
	require.Len(t, points, 1)
	assert.Equal(t, "unknown", metricAttributeString(t, points[0].Attributes, "outcome"))
}
