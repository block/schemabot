...
run schema changes. Authentication checks who is making a request;
...
group names with your own values, then restart the server after changing its
authentication settings.

You only need to follow the setup that fits your installation. The later
sections explain the details if you need them.
...
or help from the person who manages it. Register a **public client** for the
...
- **With OIDC**, use a signed JWT token for the configured issuer and audience.
  ...
  because a service token with an admin group can write. Ask your provider
  whether it can issue a compatible token for a service account.
