...
This is where SchemaBot is going. It is not a description of what SchemaBot does today, and it is not
a roadmap: there are no dates here and nothing below is a commitment to ship anything. The point of
writing it down is to have something to check individual decisions against.

For what is actually true right now, read [invariants.md](./invariants.md), which is the registry of
what must never be false while SchemaBot is running and where each rule is enforced. See
[engines.md](./engines.md) for the capability matrix and [postgresql.md](./postgresql.md) for the
current PostgreSQL envelope. Where this document and those disagree, those are right.

That separation is deliberate. The registry is only worth anything if every entry describes behavior
that actually holds, so aspiration needs somewhere else to live. Here.
