...
> An agent adds a column to a `CREATE TABLE` file and opens a pull request. The table holds tens of
> terabytes.
>
> SchemaBot works out the DDL, says on the pull request that this one takes three weeks and why,
> starts copying, slows itself down every time the database gets busy, and swaps to the new table at
> 3am on Tuesday, because that is when the author said traffic would be low. Nobody is awake for it.

The agent wrote the change. SchemaBot decided how to run it. Nobody had to know the difference
between a change that takes a millisecond and one that takes three weeks, and that is the whole idea.
