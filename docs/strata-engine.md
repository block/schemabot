...
> **Experimental:** Block currently uses Strata for sharded MySQL. It is not yet
> available for open-source use; open-source availability is planned. Existing
> deployments must set `experimental-strata-enabled: true` in the server config.
