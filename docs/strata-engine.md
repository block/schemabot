...
> **Experimental:** The Strata integration is not yet available for open-source
...
On a server with the experimental integration available, enable it before
registering a database with `type: strata`:

```yaml
experimental-strata-enabled: true
...
With the server opt-in enabled, flipping an existing `type: mysql` database to
`type: strata` requires no other database registration change: the database name, schema files, allowed dirs/repos,
