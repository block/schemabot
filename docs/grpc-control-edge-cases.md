...
| 21 | Spirit checkpoint resume loses prior copy progress after restart | Keep cutover blocked because the sentinel is present, but surface the lost-progress reason through Spirit resume status APIs instead of treating sentinel presence as proof that prior row-copy progress was reused | Follow-up after Spirit dependency exposes the API |
