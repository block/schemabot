...
| 21 | Spirit checkpoint resume loses prior copy progress after restart | Keep cutover blocked because the sentinel is present; while waiting for Spirit resume status APIs, render row-copy progress below 100% as recovery re-copying rows instead of implying cutover reattach is nearly done | Covered for current UX; typed lost-progress reason remains a follow-up after Spirit dependency exposes the API |
