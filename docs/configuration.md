...
postgres:
  ...
  concurrent_index_max_duration: 24h
...
The size check still runs when missing privileges have already blocked the
table's rewrite steps. An oversized table's blocked verdict reports the
privilege and size causes together, so both remedies are visible in the same
plan comment. If that diagnostic size lookup fails, the existing privilege
verdict remains blocked and the operational failure is logged instead of
adding an unknown-size cause.
...
`experimental-strata-enabled` is a server-only boolean, defaulting to `false`.
When false, Strata database registrations are rejected and setup comments do
not offer it. Existing deployments using Strata must explicitly enable it
before upgrading. A repository's `schemabot.yaml` cannot enable this setting.

Enabling the flag permits registrations and adds an experimental option to
setup guidance. It does not make the integration available for open-source
use. See [Strata availability](strata-engine.md).

```yaml
experimental-strata-enabled: false
```

With this configuration, registering a Strata database returns an error naming
the database and the server setting required to enable it.
