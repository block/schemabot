...
A schema change usually has two moments: the code that describes it merges,
and the database is altered. In a deploy-time model those happen in that order,
with the change running as part of the release. SchemaBot runs them the other
way around. The change is applied to the database **from the open pull
