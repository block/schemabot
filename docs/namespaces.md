...
- Because the entry reaches every namespace, so does the contradiction rule below: a table one namespace declares cannot be ignored in another. On an engine whose namespaces are database schemas, ignoring `orders` while a second schema still declares `orders` refuses the plan rather than withholding it from only the first. The error names the declaring namespace, which is often the one the operator was not thinking about — there is no way to scope an entry to a single namespace, so the fix is to stop declaring the table or to stop ignoring it.
...
- An entry that withheld nothing while the plan proposes dropping the very table it names is refused, not reported. The target holds that table, so a planner shown the exclusion would have withheld it: the only way to reach that shape is a deployment whose data plane predates `ignore_tables` and discarded the field. Reviewing it as a drop would undo the exclusion through the change it was written to prevent.
- A table that is both ignored and declared by a schema file is an error. The plan fails rather than picking one of the two, because either choice is wrong: withholding it would leave the declaring file unreconciled, and honoring the file would manage a table the config says to leave alone. Remove every `ignore_tables` entry the error names, or delete the declaring file.
- That error ignores case, which is the one place case is not significant. Whether two spellings of a name are the same table is the target's answer, not the config's: identifiers fold to lower case wherever a database is configured to store them that way, so a file declaring `Orders` and an entry naming `orders` are a contradiction there and two distinct tables elsewhere. The two directions fail differently, so they are not treated the same — withholding stays exact, because an entry must never withhold a table it does not name, while a contradiction is refused whatever the case, because letting a real one through costs an apply that fails part way. The error names both spellings, and every entry that folds to the declared name — a config that spells one table several ways is resolved only by removing all of them.
- `ignore_tables` is matched against the target's catalog before any exclusion the planner applies on its own, so an entry naming a table the engine would have excluded anyway (an archive-named table, for instance) is still reported as withheld by the config rather than as an entry that matched nothing. Which exclusion a table falls under is what the disclosure reports, so the config's own entries resolve first on every engine.
- On the MySQL-family engines, resolving the config first has a cost, and it is paid for the *shape* of an entry rather than for a match. Naming one archive-shaped table — `<name>_archive_YYYY`, with an optional month and day — makes the planner read every archive table's definition on that target, because the engine can no longer let the loader skip them before it knows which one the config meant. The entry does not have to match anything for this to happen, and the convention it matches is the one that daily or monthly partition rotation produces, so on a target that rotates partitions the extra reads can be substantial and a table that cannot be read fails the plan instead of being skipped. Prefer naming the table you mean; if you want an archive-named table disclosed as withheld by the config, that is the trade you are making.
