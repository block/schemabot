...
| `Copy`          | `status.CopyProgress` | Runner-wide `RowsCopied` / `RowsTotal`, summed from the same snapshot that fills `Tables[]`, so the two always reconcile. Zero until the copy chunker exists, and it keeps its final reading through the later phases. `RowsTotal` is a table-statistics estimate that Spirit re-reads as the copy runs, so it moves in both directions and `RowsCopied` can exceed it. SchemaBot renders per-table counters and does not read this field; it is what makes the copy fraction in `Summary` a structured value rather than text only. |
