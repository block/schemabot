...
- **Use `ddl.ClassifyStatement()` for DDL type detection.** Returns `(statement.StatementType, tableName, error)` — handles the `statement.Classify()` boilerplate (nil check, empty results). Use Spirit's typed constants (`statement.StatementCreateTable`, `statement.StatementAlterTable`, `statement.StatementDropTable`) for branching. Use `ddl.ClassifyStatementAST()` only when you need the string representation for storage/API fields. Convert between types and strings at boundaries via `ddl.StatementTypeToOp()` / `ddl.OpToStatementType()`.
