...
- **It establishes a new invariant.** Add the entry in the same PR. Give it the next ID in its family, follow the existing format (rule, *Breaks if violated:*, *Enforced:*), and prefer a completeness test over the relevant registry to a hand-maintained list, so the next regression fails CI instead of review.
