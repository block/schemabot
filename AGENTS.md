...
- **Metrics are for what can go haywire; logs are for what is rare.** Add a metric when the thing it measures has enough volume that a rate, a trend, or a threshold means something, and when you would actually alert on it — retry storms, claim contention, throttling, error rates on a hot path. Do not add a counter for a rare branch just because it is dangerous: a counter that reads zero for weeks has no baseline to alert against, and an increment tells an operator far less than the `slog.Warn` or `slog.Error` beside it, which carries the identifiers, the affected objects, and the reason. Log the rare dangerous branch well and leave it at that; a log-based monitor can still count it if that day ever comes. When a metric does earn its place, the expected operator action should be obvious from its name, attributes, and nearby code comments or docs.
