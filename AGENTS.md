...
- **One owner closes a handle; redundant closers must not assert.** A handle (`*sql.DB` etc.) passed to a constructor that closes it in its own `Close()` is owned by that component — e.g. a store built with `New(db)` closes `db` when the service holding the store is closed. Assert `Close()` only on that sole owner. If a second site also closes the same handle — commonly a test's open-time `t.Cleanup`, registered so an early failure can't leak the handle — the two closers double-close it and the later one returns `sql: database is already closed`. That redundant close must discard its error (`_ = x.Close()`); never assert it, or a guaranteed double-close becomes a spurious teardown failure. This is the one sanctioned exception to the "never `_ = x.Close()`" rule above: prefer a single owner per handle, and use a redundant discarded close only for early-exit leak safety.
